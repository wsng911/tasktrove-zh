...
export async function GET(
  ...
  const handler = createAssetHandler(["public", ...path], { requireAuth: false })
...
export async function POST(
  ...
  const { path } = await params
  const handler = createAssetMutationHandler(["public", ...path], "create", {
    requireAuth: true,
...
export async function PATCH(
  ...
  const { path } = await params
  const handler = createAssetMutationHandler(["public", ...path], "update", {
    requireAuth: true,
    allowedExtensions: [...DEFAULT_ASSET_ALLOWED_EXTENSIONS],
    maxBytes: DEFAULT_ASSET_MAX_BYTES,
  })
...
export async function DELETE(
  ...
  const { path } = await params
  const handler = createAssetMutationHandler(["public", ...path], "delete", {
    requireAuth: true,
  })
  return handler(request)
}
